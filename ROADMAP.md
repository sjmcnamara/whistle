...
### v1.11.2 — Relay-delivery-order commit buffering ✅
_Released 2026-09-22_

- **(iOS & Android) MLS commits could reach MDK out of the order they were published, permanently stranding a device one epoch behind.** Closes the "MLS commits are applied in relay-delivery order, not epoch order" Deferred entry below. `handleIncomingEvent` handed kind-445 events to MDK as they arrived, with no ordering guarantee — NIP-01 doesn't promise chronological backlog replay, and multiple relays fan in to one subscription. Fixed by buffering kind-445 events per subscription until the relay signals end-of-stored-events, then replaying them sorted by `created_at` (ties broken by arrival order — `created_at` only has 1-second resolution, so same-second bursts can still tie) before any of it reaches MDK. Live post-catch-up delivery is left unbuffered, deliberately — this is a location app, and adding latency to every chat/location message would trade away real responsiveness for a reordering risk that's rare outside backlog replay.

    **Correction while building the regression test**: the original Deferred entry (and this project's own mental model) assumed a retried out-of-order commit would eventually hit MDK's `previouslyFailed` blacklist. Reproducing it directly against two real MDK instances shows the actual behavior is narrower but just as terminal for this case: the first delivery of an out-of-order commit throws (a decrypt failure — wrong epoch's exporter secret — not a typed result, a separate gap the "thrown decrypt exceptions" Deferred entry below covers), and retrying the same message after its predecessor lands returns `unprocessable` again rather than succeeding or ever reaching `previouslyFailed`. Same practical conclusion (no path back to a correct state without the ordering fix), different mechanism than assumed.

    **Known gap**: a burst of commits published within the same wall-clock second (e.g. rapid-fire promote/demote/remove, exactly the burn-flow shape that originally prompted this investigation) can't be reordered by this fix — `created_at`'s 1-second resolution can't distinguish them, and there is no other plaintext ordering signal available pre-decryption. Cross-second reordering (backlog replay after being offline, multi-relay skew) is what this actually protects against.
...
- **MDK 2.0 / MarmotKit migration** _(planned major version, blocks nothing yet — plan confirmed 2026-09-23, **iOS spike in progress** on `feature/v2.0-marmotkit-spike`)_: we are pinned to `mdk-swift` at MDK 0.8.0 (frozen, archived 2026-08-05, protocol v1 — deprecated per upstream). Upstream's rewrite is done and published: **MarmotKit `marmotkit-v0.10.4`** (tagged 2026-09-20) runs Marmot protocol v2 and is a real, consumable release (Swift package + Kotlin bindings), not just an announcement to wait for.
    ...
    **Confirmed directly with upstream** ([mdk#938](https://github.com/marmot-protocol/mdk/issues/938)) and cross-checked against MarmotKit's own `API-REFERENCE.md` rather than taken on paraphrase:
    ...
    - **Relay config is not fully hidden inside MDK** — the app still owns relay *selection* via `set_account_nip65_relays`, `set_account_inbox_relays`, `publish_relay_lists`; MDK owns only the WebSocket/dial/subscribe mechanics. Our Advanced Settings "Relays" UI can stay as-is, calling MDK's setters instead of driving `RelayService`'s own connection pool.
    - Admin API is a strict superset of ours (`promote_admin`, `demote_admin`, `self_demote_admin`, `remove_members`, `leave_group`, `disband_group`, `update_group_profile`, `update_group_image`) — the MIP-03 rules we hand-enforce (self-demote-before-leave, last-admin guard) are presumably enforced inside MDK now.
    - MDK has its own fork/recovery machinery — `quarantined_groups`, `retry_hydrate_quarantined_group`, `group_recovery_status`, `confirm_group_rejoin`, `retry_group_convergence`. **If this works as documented it makes `GroupHealthTracker`, our soft/hard-resync paths, and the v1.11.2 relay-delivery-order buffer obsolete** — a claim to verify with an adversarial two-device test (the kind that caught the v1.11.1 burn bug), not accept on faith, before deleting any of that safety net.

    **What breaks — no code-only migration path**:
    - **The join flow has no equivalent.** Today: prospect scans an admin's invite QR → publishes a KeyPackage → sends an encrypted join-request rumor → admin approves → admin calls `addMembers`. MarmotKit has no outside-group messaging API (`send_custom_event` requires an existing group already). Danny's recommended fix: **reverse the QR** — the prospective member shows a QR of their own npub (once their KeyPackage has published), the admin scans it and calls `invite_members` directly. This is a real product UX change on both platforms (`JoinGroupView.swift`, `InviteShareView.swift`, `QRScannerView.swift`), not a code port — `PendingInviteStore` and `JoinRequestStore` likely disappear entirely since there's no more "pending request" state to track.
    - Onboarding sequencing changes: account setup returns "local ready" before relay publication completes, so "show my invite QR" needs to gate on KeyPackage-published, not just identity-created.
    - **0.8 groups do not migrate** — already agreed with Danny in the issue thread; every device on a group needs the new build and users recreate groups, same shape as the "Dublin group" recovery communication.
    ...
    **Planned sequencing**:
    ...
    2. ✅ **Map our existing payload kinds against MDK's reserved-kind rejection list** — `chat=9` collides: MDK reserves kind 9 for its own CHAT (confirmed against source, not docs — `RESERVED_APP_EVENT_KINDS`/`validate_custom_event_kind` in `crates/marmot-app`, purely value-based, no call-path exception; full 16-value reserved table in CLAUDE.md's MDK 2.0 section). `location=1` and `leaveRequest=2` are clear. **Target v2.0 numbering: `chat` renumbered to `3`** — deliberately NOT applied to `WhistleCore/MarmotKind.swift` yet, since that constant is live in the shipping 0.8-era protocol; renumbering it before the actual cutover would desync chat rendering between pre/post-update clients on existing groups, for no MarmotKit-related reason. Apply it together with step 3's rewrite, as part of the one breaking v2.0 change.
...
- **Auto-trigger hard resync on durable epoch lag** _(idea, iOS & Android, not yet implemented)_: now that `GroupHealthTracker` can see both `previouslyFailed` and `unprocessable` failures (v1.8.13) and relay-delivery-order commits are buffered before dispatch (v1.11.2), a device can still end up genuinely stuck — e.g. it missed a commit entirely rather than merely receiving it out of order. Worth auto-triggering the existing hard resync (remove+re-add) once a device is detected durably behind the group's leading epoch, instead of requiring a human to notice and tap the manual Resync button.
