...
- **(iOS & Android) Root cause found for the v1.10.2 stale-admin symptom, and a second, more actionable bug alongside it.** Revealing the npub confirmed the `Dublin` group's cached "Stephen"/Admin row really is a different, old pubkey — not the user, as suspected. But then attempting `Leave Group` (now working correctly since v1.10.0, for a *confirmed non-admin*) threw MDK's raw error instead: `"Admins must self-demote before leaving. Use self_demote() first."` — proving the user's *current* identity is also live-admin in Dublin's real MLS state, something the app's cached admin list never reflected (it only ever showed the one stale "Stephen" entry). First attempt called MDK's `syncGroupMetadataFromMls()` (refreshes cached metadata from live state, previously unused anywhere) before the admin check — **confirmed on the affected device that this does not fix an already-diverged group**: the leave attempt threw the identical raw error even with the sync in place. `leaveGroup` no longer decides admin status from any cached read at all — it unconditionally attempts `selfDemote` first and interprets MDK's own authoritative response (distinguished by message: `"only admins can perform this operation"` vs `"last active admin"`) instead of pre-deciding from `getGroup().adminPubkeys`.

    **Still open**: how the cache actually diverged from live state in the first place, how a second, unaccounted admin pubkey ended up live-admin in a group alongside the user, and a related `previouslyFailed` diagnostics entry suggesting this device may be behind the group's true leading epoch (a known out-of-order-commit issue, see the MLS commit ordering item below) are all unresolved — most likely inherited from this identity's history predating today's fixes. Group Detail's own admin-status *display* still reads the same cache and can remain visibly wrong for an already-diverged group like this — only the ability to actually leave is fixed here. If the device is genuinely behind the group's leading epoch, the published self-demote/leave commits risk being built on stale state that other members' devices could reject — verify by having another member confirm the departure actually registered on their end, not just that it succeeded locally.

### v1.10.4 — Root cause: silent identity swap from the v1.8.7 rename, plus closing the admin-cache UI gap ✅
_Released 2026-09-18_

- **(iOS) Actually found the root cause of the whole `Dublin` saga: the v1.10.2/1.10.3 investigation was chasing a symptom, not the disease.** MLS forward secrecy (a forked/behind-epoch device cannot decrypt a later-epoch message) ruled out a true fork — the affected device successfully decrypted a group member's location update sent 30 seconds earlier, so it was never behind. The real mechanism: the v1.8.7 bundle-id rename (`#203`) removed the pre-rename Keychain access group from `Whistle.entitlements` outright instead of keeping both during a transition. On that device's first v1.8.7+ launch, the nsec stored under the old access group became unreachable; `IdentityService` read "no nsec" as "first launch" and silently generated a brand-new identity, no warning shown anywhere. `whistle.db` isn't Keychain-scoped, so it was untouched — every existing group kept running under the *old* identity's MLS leaf and its fixed-at-creation credential (still labeled with the old device nickname, e.g. "Stephen"), while the app itself moved on to a new outward identity that MLS never re-credentials a leaf to match. `IdentityService` now treats "no nsec, but real local MLS data already on disk" as a distinct anomaly rather than a first launch — it refuses to auto-generate a replacement identity and instead blocks on a new screen requiring an explicit choice.

    **Same-day correction, confirmed live on the affected device**: the first attempt at this fix also widened `Whistle.entitlements` to list the pre-rename access group indefinitely alongside the current one, meaning to make the old, orphaned identity reachable again. This broke the *live*, working account it was meant to protect — installed on the reporting user's own main phone, it surfaced an unfamiliar dormant identity with an empty group list and MDK threw `Storage("Wrong encryption key")` trying to open the real database. Cause: neither `KeychainService`'s queries nor MDK's own internal keyring-core Keychain usage pin a specific access group; with two groups listed and a dormant pre-rename item still sitting in Keychain for this account, an unscoped query became genuinely ambiguous and picked the wrong item for both the nsec and the database's encryption key. No data was destroyed — MDK correctly refused to delete the now-undecryptable database, and the misfiring read path never writes — and reverting `Whistle.entitlements` back to a single access group and reinstalling immediately restored the account (confirmed: 3 groups, correct identity, database decrypted cleanly). The anomaly-detection screen doesn't depend on the old group being reachable, so it stays in place without this risk; recovering the dormant old identity, if ever wanted, needs a deliberate one-time tool that queries the old access group by name, not an ambient entitlements change.
- **(iOS & Android) Closed the last gap in the v1.10.3 admin-cache fix: "Invite People" (`Add by npub`), "Ready to Join" approvals, and group rename were still gated on the same stale cache that leaving and member-row actions were already fixed to ignore.** Caught directly from the affected device: leaving worked (v1.10.3) but there was no way to use the one recovery path that actually mattered — adding the current, genuinely-live identity back into the group as a fresh, correctly-recognized member — because the exact button needed to do that ("Add by npub") was hidden behind `viewModel.isAdmin`. All four are now unconditional; the underlying MDK calls still enforce and fail safely for a genuine non-admin.

    **Kept from earlier v1.10.x commits on this fix, after re-review**: `leaveGroup`'s cache-free `selfDemote`-first rewrite (v1.10.3) stays as the correct model for member-initiated MLS operations generally. `promoteToAdmin`'s "always include your own pubkey when writing the admin list" fix (v1.10.3) stays — it's a real hardening against the general class of bug even though MDK's own `prune_and_validate_admin_update` silently drops a non-member entry, so it's a no-op for `Dublin`'s specific case. The `syncGroupMetadataFromMls()` call added to `GroupDetailViewModel.load()` (v1.10.3) stays too — reading MDK's real source confirmed it's still correct and useful for keeping the normal case fresh; it was never wrong, just insufficient on its own for a group whose *live* MLS extension genuinely lacks the user's current identity, which is exactly why the cache stopped being trusted for the leave/invite/join decision points above instead.

    **Known remaining gap, deliberately deferred**: the group-photo picker's admin gate and `MarmotService.isAdmin(_:ofGroup:)` underneath it still read the same stale cache. Lower stakes than being locked out of leaving or inviting, and tangled with a real policy question (should any member be able to set the group photo?) not worth deciding as a side effect here.
- **(iOS & Android) Two more blockers found by actually walking the `Dublin` recovery to completion, both fixed same-day.** First: `MarmotService.addMember` had a standalone `guard memberHex != publicKeyHex else { throw .alreadyMember }` — refusing to even attempt adding the app's own current pubkey, before ever checking MDK's real member list. It surfaced as "this person is already a member of the group," which was never true; the check was always redundant in the healthy case (the real membership check just below catches it anyway) and only had teeth in exactly the recovery scenario this release exists for, where it wrongly blocked the one thing needed. Removed. Second: after that fix let the recovery add-by-npub through, promoting the newly-added "Me" entry to admin was itself hidden — `MemberRowView`'s swipe actions gated "Make Admin" on `!member.isMe` alongside "Resync"/"Remove", so the one row that needed promoting to finish the recovery had no way to be promoted. Split the gating: "Resync" and "Remove" correctly stay hidden on your own row (self-resync isn't coherent, self-removal has its own "Leave Group" flow), but "Make Admin" no longer excludes your own row — MDK's promote only cares that the promoter's leaf is a real admin and the promotee a real member, not whether they're "the same app identity." Confirmed live end-to-end on the affected account: `Dublin` now reports `memberCount: 5`, `adminCount: 5`, with the recovered identity a genuine admin.
...
- **Android background execution has no durable story at all** _(reliability, Android — found alongside v1.8.13)_: a 4-agent deep dive into a 3-device sync test found that Android has zero infrastructure to survive backgrounding. v1.8.14 fixed the cheapest gap (catch-up calls not running on resume). **v1.8.16 fixed the three largest remaining gaps**, prompted by a live GrapheneOS report ("halts within minutes of closing and does not run on startup... you can never see anyone's position unless they have the app open"):
    ...
    - No `WorkManager` anywhere in the module — no OS-scheduled fallback of any kind. Lower priority now that a foreground Service exists; would mainly help the rare case where the OS kills the process despite foreground status.
