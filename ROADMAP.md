...
- **Android background execution has no durable story at all** _(reliability, Android — found alongside v1.8.13)_: a 4-agent deep dive into a 3-device sync test found that Android has zero infrastructure to survive backgrounding. v1.8.14 fixed the cheapest gap (catch-up calls not running on resume); these remain:
    ...
    - `ensureSubscriptionsActive()` (added in v1.8.12) only restarts the subscription coroutine if `subscriptionJob?.isActive == false`. If Doze/Samsung freezes the process without cancelling the coroutine, the check false-positives — the app looks alive to itself on resume and does nothing: no reconnect, no resubscribe. (v1.8.14's foreground catch-up sweep runs regardless of this check, so it papers over the symptom for the three catch-up calls specifically, but the underlying false-positive is still there for anything else gated on it.)
    ...
    Fix candidates, roughly in order: fix `ensureSubscriptionsActive()` to check `relay.hasConnectedRelays()` instead of coroutine `isActive`; request `ACCESS_BACKGROUND_LOCATION` at runtime; add a real foreground `Service` for relay + location; request the battery-optimization exemption with explicit Samsung guidance.
