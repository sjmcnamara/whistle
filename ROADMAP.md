...
- Same session surfaced several other Android-specific gaps not yet fixed: no foreground service / WorkManager / battery-optimization exemption (nothing survives Doze or Samsung's background killer), `ensureSubscriptionsActive()`'s coroutine-`isActive` check false-positives when the process is frozen rather than killed, the gift-wrap catch-up (`fetchMissedGiftWraps`/`catchUpGroup`/`rotateStaleGroups`) only runs at cold start rather than on `onForeground()`, and `ACCESS_BACKGROUND_LOCATION` is declared in the manifest but never requested at runtime. Tracked as follow-up work, not part of this release.
...
- **Android background execution has no durable story at all** _(reliability, Android — found alongside v1.8.13)_: a 4-agent deep dive into a 3-device sync test found that Android has zero infrastructure to survive backgrounding, independent of the v1.8.13 health-tracker fix:
    - No foreground `Service` exists despite `FOREGROUND_SERVICE`/`FOREGROUND_SERVICE_LOCATION` being declared in the manifest — dead permissions. `RelayService`/`MarmotService`/`LocationService` are ordinary Hilt singletons with no lifetime independent of the app process.
    - No `WorkManager` anywhere in the module — no OS-scheduled fallback of any kind.
    - No battery-optimization exemption request (`ACTION_REQUEST_IGNORE_BATTERY_OPTIMIZATIONS`) — the app never asks to be exempted from Doze/App Standby, and Samsung's separate "sleeping apps" battery manager isn't covered by that exemption even if it were requested.
    - `ensureSubscriptionsActive()` (added in v1.8.12) only restarts the subscription coroutine if `subscriptionJob?.isActive == false`. If Doze/Samsung freezes the process without cancelling the coroutine, the check false-positives — the app looks alive to itself on resume and does nothing: no reconnect, no resubscribe, no catch-up.
    - `fetchMissedGiftWraps()` / `catchUpGroup()` / `rotateStaleGroups()` only run at cold-start `onAppear()`, never from `onForeground()` — a missed "ready to join" gift-wrap is only reliably recovered by a full app relaunch, not a resume.
    - `ACCESS_BACKGROUND_LOCATION` is declared in the manifest but never requested at runtime (`MapScreen.kt` only asks for fine/coarse) — location updates can silently degrade to foreground-only independent of whether the OS ever touches the process.
    - `AppViewModel.onCleared()` stops subscriptions/location but never calls `relay.disconnect()`, which can leave the websocket client half-torn-down.

    Fix candidates, roughly in order: wire the three catch-up calls into `onForeground()` (cheapest, likely fixes the joiner-visibility symptom outright); fix `ensureSubscriptionsActive()` to check `relay.hasConnectedRelays()` instead of coroutine `isActive`; request `ACCESS_BACKGROUND_LOCATION` at runtime; add a real foreground `Service` for relay + location; request the battery-optimization exemption with explicit Samsung guidance.
