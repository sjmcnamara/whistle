...
- **(iOS) Crash while panning/zooming the map**: map pins rendered `MemberAvatarView`, which reaches for `MemberAvatarStore` via `@EnvironmentObject`. MapKit hosts `Annotation` content in its own `_UIHostingView` with none of the root environment, built from `MKAnnotationManager.updateVisibleAnnotations` (a timer callback outside SwiftUI's update pass) — so the lookup trapped and killed the app with `EXC_BREAKPOINT` in `EnvironmentObject.error()`. `MemberPinView` now takes a resolved `UIImage?` from `MapView` and reads nothing from the environment. Latent since v1.7.1, reported from the field on 1.8.1 / iOS 26.6. Android was structurally immune (`MapScreen` already passes a resolved bitmap per pin).
