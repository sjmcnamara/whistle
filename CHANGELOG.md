...
### Fixed
- **(iOS) Root cause found for the `Dublin`-style "stale admin"/ghost-identity reports: a silent identity swap caused by the v1.8.7 bundle-id rename, not a live fork or a cache bug.** That rename (`#203`) dropped the old `keychain-access-groups` entitlement entirely instead of keeping both during a transition. On a device's first launch of v1.8.7+, the old Keychain access group — and the nsec stored in it — became unreachable; `IdentityService` saw "no nsec found" and treated it as a genuine first launch, silently generating a brand-new Nostr identity with no warning. The MLS database (`whistle.db`) isn't Keychain-scoped, so every existing group kept working under the *old* identity's MLS leaf and credential, indefinitely — the app just stopped recognizing that leaf as "you". `IdentityService` now distinguishes a genuine first launch from this anomaly (no nsec, but real local MLS/group data already on disk) and refuses to auto-generate a replacement identity in that case, instead surfacing a blocking screen that requires an explicit, deliberate choice ("Try Again" or "Create New Identity Anyway") rather than silently swapping identities under the user.
  - **A same-day correction to this fix's own first attempt, confirmed live on the affected device**: the initial version also added the pre-rename access group back into `Whistle.entitlements` alongside the current one, meaning to make the old, orphaned identity reachable again. Reverted after it broke the *live* account it was meant to protect — `KeychainService`'s save/load/delete queries (and MDK's own internal keyring-core Keychain usage, which isn't ours to control) never pin a specific `kSecAttrAccessGroup`. With a single access group that's harmless; with two, any account that still has a dormant pre-rename Keychain item under the old group becomes ambiguous, and an unscoped query can resolve to either one. Confirmed live: it picked the wrong item for both the nsec *and* MDK's database encryption key simultaneously, throwing `Storage("Wrong encryption key")` and surfacing an unfamiliar, dormant identity with an empty group list. No data was actually lost — MDK correctly refused to delete the undecryptable database rather than discard it, and the nsec-load path that misfired never writes — but it was a real, live break of a working account, not merely displaying a stale value. Dropped back to one access group; the anomaly-detection screen above doesn't depend on the old group being reachable, so it stays protective without this risk.
...
- **(iOS & Android) Two more blockers found live while actually walking the recovery path end to end on the affected `Dublin` account, both fixed the same day**: `MarmotService.addMember` refused outright to add the app's own current pubkey (`guard memberHex != publicKeyHex else { throw .alreadyMember }`), before ever checking MDK's real member list — surfacing as "this person is already a member of the group" even though they demonstrably weren't. That guard was always redundant in the healthy case (a real member is caught by the very next check anyway) and only had an effect in exactly the abnormal case this release exists to fix, where it was actively wrong. Removed. Second: once added, promoting that new "Me" entry to admin was itself hidden — `MemberRowView`'s swipe actions hid "Make Admin" on your own row (`!member.isMe`), a reasonable-looking guard that happened to block the one row that needed promoting to finish the recovery. Split out from the row's "Resync"/"Remove" actions, which correctly stay self-hidden, and left "Make Admin" available on any non-admin row including your own — confirmed live: `Dublin` now reports `memberCount: 5`, `adminCount: 5`, with the recovered identity a genuine admin.
...
- **(iOS) The old, pre-v1.8.7 identity is still permanently unreachable from within the app.** It's confirmed to still exist, dormant, in Keychain — recovering it safely would need a one-time tool that explicitly queries the old access group by name rather than widening `keychain-access-groups` ambiently. Not needed for `Dublin`: the plan there is to add the *current* identity as a fresh member via "Add by npub" and promote it, not to resurrect the old one.

## [1.10.3] — 2026-09-17

### Fixed
- **(iOS & Android) `Leave Group` could throw a raw MDK error instead of self-demoting when it should have.** Root cause of the v1.10.2 "known open issue": our cached copy of a group's admin list (`Group.adminPubkeys`, read by `getGroup()`) can drift from what MLS itself actually enforces — a member's admin status can be live in the group's real MLS state without our local cache ever picking it up. `leaveGroup`'s admin check read the stale cache, wrongly concluded the caller wasn't an admin, and skipped straight to a plain `leaveGroup()` call — which MDK then rejected outright with `"Admins must self-demote before leaving"`, since it enforces that rule against the real, live admin list regardless of what our cache says. Confirmed on a real affected account that MDK's own `syncGroupMetadataFromMls()` (refreshes cached metadata from live state) does **not** reliably fix this for an already-diverged group — so `leaveGroup` no longer trusts the cache at all: it now unconditionally attempts `selfDemote` first and interprets MDK's own authoritative response (admin → demoted then leaves; not an admin → falls through to a plain leave; sole admin → a clear error asking to promote someone else first) rather than pre-deciding from a cached read. Group Detail's own admin display still reads the same cache and can remain visibly wrong for an already-diverged group like this — only the ability to actually leave is fixed here.
